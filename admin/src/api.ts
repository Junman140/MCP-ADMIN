...
export async function apiCBT<T>(path: string, opts: RequestInit = {}): Promise<T> {
  ...
  // For CBT, we pass tenantId in headers instead of query param for Go middleware
  const tenantId = getSelectedTenantId();
  if (tenantId) headers["X-Tenant-ID"] = tenantId;

  const res = await fetch(`/cbt-api${path}`, { ...opts, headers });
  if (!res.ok) {
